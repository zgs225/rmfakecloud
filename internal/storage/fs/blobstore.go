...
func createMetadataFile(metadata common.MetadataFile) (r io.Reader, filehash string, size int64, err error) {
...
func (fs *FileSystemStorage) LoadBlob(uid, blobid string) (reader io.ReadCloser, gen int64, size int64, err error) {
	...
	log.Debugln("LoadBlob Fullpath:", blobPath)
	if blobid == rootBlob {
...
func (fs *FileSystemStorage) GetBlobMetadata(uid, docId string) (*common.MetadataFile, error) {
	tree, err := fs.GetCachedTree(uid)
...
func (fs *FileSystemStorage) UpdateBlobMetadata(uid, docId string, md *common.MetadataFile) error {
	tree, err := fs.GetCachedTree(uid)
	if err != nil {
		return err
	}
	...
	err = updateTree(tree, blobStorage, func(t *models.HashTree) error {
		...
		mdReader, mdHash, mdSize, err := createMetadataFile(*md)
		if err != nil {
			return err
		}

		err = blobStorage.Write(mdHash, mdReader)
		if err != nil {
			return err
		}

		mdHashEntry := models.NewHashEntry(mdHash, docId+storage.MetadataFileExt, mdSize)
		mdHashEntry.Size = mdSize

		// New hash doc
		newHashDoc := models.NewHashDocWithMeta(docId, *md)
		...
		for _, entry := range hashDoc.Files {
			if strings.HasSuffix(entry.EntryName, storage.MetadataFileExt) {
				continue
			}
		...
		hashDocReader, err := newHashDoc.IndexReader()
		if err != nil {
			return err
		}

		err = blobStorage.Write(newHashDoc.Hash, hashDocReader)
		if err != nil {
			return err
		}
